...
def timeseries(df):
    # number of projects
    ts = (
        df.set_index(pd.DatetimeIndex(df["launched_at"]))
        .sort_index()
        .usd_pledged.resample("MS")
        .count()
    )
...
def barplots(df):
    ...
    sns.barplot(
        freq, y="freq", x="state", hue="state", hue_order=order, order=order, ax=ax9
    )
        ...
        if thresh is not None:
            for t in ax.texts:
                if abs(float(t.get_text())) >= thresh:
                    t.set_text(
                        t.get_text()
                    )  # if the value is greater than _thresh_ then I set the text
                else:
...
def run(df, *args):
    ...
    histplots(
        ...
        bins="doane",
        cols=2,
        xlog=[False, False, True, False, True],
    )

    bar_by_feature(df, "category", doSort="SuccessRate", palette="flare")
    bar_by_feature(df, "country", palette="flare")
    bar_by_feature(
        df, "deadline_day", doSort=list(calendar.day_name)[0:], palette="flare"
    )
    bar_by_feature(
        df, "deadline_month", doSort=list(calendar.month_name)[1:], palette="flare"
    )
    ...
    corr_matrix(df, corr_feat, thresh=0.1)
    corr_matrix(df, corr_feat, col="state", thresh=0.1)
